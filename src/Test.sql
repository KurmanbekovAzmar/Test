...
--DDL Task
alter table agencies alter name set not null ;
alter type familystatus rename to MartialStatus;
...
alter table customers add constraint a unique(email);
alter table owners add constraint b unique (email);
alter table agencies add  check ('+996');
...
select  from houses join owners o on houses.owner_id = o.id where price>1500 and price <2000;
-- 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select addresses.id,(city,region,street) as adress,h.id, house_type,description from addresses join  houses h on addresses.id = h.address_id offset (4) limit(6);
-- 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select h.house_type as house_type,description,o.first_name as owner ,c.first_name as client,a.name as agient  from rent_info join agencies a on rent_info.agency_id = a.id join owners o on rent_info.owner_id = o.id join customers c on rent_info.customer_id = c.id join houses h on o.id = h.owner_id;
-- 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.
select * from customers where date_of_birth > '1999-1-1' offset 9 limit 15;
-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select house_type,rating,first_name from houses join owners o on houses.owner_id = o.id order by rating;
select house_type,rating,first_name from houses join owners o on houses.owner_id = o.id order by rating desc;
...
select name,(a.city,a.region,a.street)as address ,house_type , description from agencies join addresses a on agencies.address_id = a.id join houses h on a.id = h.address_id where name = 'My House';
-- 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select furniture,o.first_name,(a.city,a.region,a.street) as adress from houses join owners o on houses.owner_id = o.id join addresses a on a.id = houses.address_id where houses.furniture=true;
...
select avg(rating),max(rating),min( rating) from houses;
...
select * from customers where customers.nationality='Kyrgyzstan';
-- 18.Эн коп болмолуу уйду жана анын адресин ал уй кайсыл ownerге таандык ошону чыгарыныз.
select * from houses join owners o on houses.owner_id = o.id ;
